...
- `nr`：按 final moleculetype atom order 重新生成；
...
# 5. Standard-side deletion mechanical cleanup
...
> 除 final index remapping 和 confirmed atom deletion 引起的 mechanical cleanup 外，standard topology entry 的其它内容保持不变。

# 6. Standard attachment-site atom type review
...
- 使用 final map 定位 final topology replacement target；
...
↓
map all atom references to final indices
↓
...
# 11. Other molecule-level directives
...
# 13. Coordinates 与 final map

## Standard

坐标来自 2.2 all-atom structure，应用 confirmed standard-side deletions。

## Topo-linked nonstandard unit

只贡献 unit 自身最终存在的 SOURCE + ADDED_H atoms。

不贡献：

```text
2.3 standard parameterization fragment
CAP
```

## Independent nonstandard

使用 2.4 all-instance structure/map。

## FF-direct solvent / ions

当前结构中实际存在的实例贡献 coordinates；topology definition 使用 2.1 指定 definition。
...
5. 只做 final moleculetype organization 所需的块级组合，不自由重排。

linked nonstandard block 不按 attachment-site 紧跟插入 standard residue，而是在其所属 standard residue block 之后按已冻结 object order 组织。
