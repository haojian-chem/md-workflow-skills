...
本归档目录直接保存旧版 main Skill、主要科学规则、正式结果 schema、reference manifest schema 和脚本说明；`PACKAGE_TREE.md` 锁定旧 package 的完整 tree/blob identity。未在本目录重复复制的 registry、CCD seed 数据或其它未修改支持文件，可通过上述 commit/tree 精确恢复。

该旧版的主要实现特征是以固定 Python pipeline 驱动 model scope、分类、关系检查、人工决定同步和最终结果构建。新版 1.2 不应从本目录继承 runtime authority；需要历史审计时才读取。
