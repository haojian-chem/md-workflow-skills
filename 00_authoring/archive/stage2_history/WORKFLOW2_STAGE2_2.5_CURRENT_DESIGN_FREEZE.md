...
从本次实际使用的各来源 `.itp` 中收集 `[ atomtypes ]`、`[ bondtypes ]`、`[ angletypes ]`、
`[ dihedraltypes ]`、`[ pairtypes ]` 和 `[ nonbond_params ]`，统一写入一个独立 `.itp`，不拆分为多个文件。

是否属于同一参数项，由执行 Agent 按对应 GROMACS section 的实际语义判断，不再为六类参数项分别固化匹配规则。

先在收集到的参数定义之间进行去重：

- 判断为同一参数项且定义相同：只保留一份，并保留其来源信息；
- 判断为同一参数项但定义不同：视为冲突，交由用户决定，不按来源顺序自动覆盖。

完成内部去重后，再与当前体系实际采用的力场参数定义进行比较：

- 力场中不存在同一参数项：保留该定义；
- 力场中存在同一参数项且定义相同：不重复启用该定义，在独立参数 `.itp` 中以注释形式保留原定义，并注明其来源以及当前力场已存在相同定义；
- 力场中存在同一参数项但定义不同：视为冲突，交由用户决定，不自动覆盖现有定义。
...
结果记录的完整 YAML 字段结构由后续 `references/results.md` 设计确定；本节当前冻结结果文件名及必须登记的结果文件范围。
