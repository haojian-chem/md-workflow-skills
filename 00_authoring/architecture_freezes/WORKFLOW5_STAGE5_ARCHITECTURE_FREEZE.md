...
Status: **FROZEN AUTHORING BASELINE — STAGE 5 IN PROGRESS / NOT COMPLETED**

本文件保留 Stage 5 — Analysis 尚未被 current runtime package 完整接管的 authoring / architecture baseline。Stage 5 main Skill 已经 active；已实现部分的 runtime authority 为：

`05_analysis/SKILL.md`

以及 current capability inventory 中实际存在的 capability entries。Stage 5 的整体建设状态与 current entry 统一以：

`00_authoring/project_design/MD_WORKFLOW_MASTER_PLAN.md`

为准。本 freeze 不覆盖已经实现的 current runtime 规则，只继续约束尚未完成的 Stage 5 authoring / capability implementation。
...
一张 Task Sheet 只承载当前有界的 Stage 5 执行范围，不要求覆盖同一科研任务的全部分析工作。前序 Task Sheet 已形成且仍适用的正式分析结果或 prepared inputs 可以直接被当前 Task Sheet 消费，不为了流程完整复制为当前 plan items。

## 2. Manager / Stage 5 boundary

Manager 只负责：

- 判断当前 Task Sheet 的执行范围是否包含 Stage 5；
- 在当前 Task Sheet 中建立 `5 Analysis`；
- 保留用户明确提出的分析目标、对象、约束；
...
→ write the complete current plan into current Task Sheet
→ execute the plan
```

这里的“完整 plan”只指当前 Task Sheet 的 Stage 5 执行范围，不要求复制同一科研任务在前序 Task Sheet 已完成或未来 Task Sheet 才需要的全部分析工作。
...
Stage 5 main Skill 在当前 Task Sheet 的 `5 Analysis` 条目内维护局部整数编号：`1, 2, 3, ...`。
...
settings
status
path    # 当前 Task Sheet 实际执行该 item 时必填；direct reuse 且当前 Task Sheet 不执行时可省略
```
...
- `status`：`未完成 / 已完成 / 已终止`；
- `path`：当前 Task Sheet 实际执行该 item 时，记录 item 相关文件的完整目录，用于查询和恢复，不指向单个结果文件；若该 item 因 direct reuse 在当前 Task Sheet 中不执行，则可省略 `path`；
- `results`：可选字段，记录已经通过对应 capability owner validation、值得直接定位或供后续 item 消费的关键正式结果/产物入口；direct reuse 时可指向被复用的既有正式结果；
- `reason`：终止原因；direct reuse 时应说明已有正式结果已满足当前需求，并可注明其原 Task Sheet / result entry。

因此 direct reuse 虽然当前 Task Sheet 不执行该 capability，`inputs` 和 `settings` 仍然保留；它们描述的是**当前需求**，不是把旧 Task Sheet 的执行参数复制成当前执行记录。候选旧结果自己的 inputs / settings 应从其原 Task Sheet / result record 追溯，并与当前 item 比较。

当执行失败、validation 不通过或后续证据要求调整计划时，**Stage 5 不规定“必须修改原 item”或“必须终止并新建 item”的统一判据**。具体处理方式由实际使用 Stage 5 Skill 的 Agent 或用户结合当前 Task Sheet 的执行范围判断。Stage 5 只要求调整后的 Task Sheet 保持可追溯：已有编号不重排；若 item 进入 `已终止`，必须记录 `reason`；当前 plan 必须继续准确反映尚需执行的工作与有效依赖关系。

`path` 与 `results` 的边界：

```text
path    → 当前 Task Sheet 实际执行 item 的工作/文件目录与恢复定位入口
...
Stage 5 不建立类似 Stage 4 `run_unit.yaml` 的 project-level run-unit 索引。每个实际执行 Stage 5 的 Task Sheet 使用自己的 analysis 工作目录：

```text
<project_root>/05_analysis/<task_id>/
```

其中 `<task_id>` 是当前 Task Sheet 的 `Txxxx` 标识，例如 `T001`。

当前 Task Sheet 内实际执行的 plan item 默认使用：
...
该目录即普通实际执行 item 的默认 `path`。如果 direct reuse 导致当前 item 不执行，则不创建无用 item 目录，`path` 可省略。
...
`trjconv` 属于当前已知需要对 processed trajectory 做额外集中管理和登记的 capability：Stage 5 保留当前 Task Sheet 的 plan item / 工作记录，但 trajectory 的集中存放、允许登记的 trajectory 文件以及登记动作由 `trjconv` capability owner 定义。
...
Stage 5 capability inventory 持续维护，没有固定 completion catalog。当前已实现条目以正式 inventory 为准；尚未实现的 capability 只有在形成实际可引用的 `entry` 后才能加入 inventory。
...
如果当前分析需求没有合适的已登记 capability，Stage 5 不为该缺失能力创建对应的 Stage 5 plan item，也不创建虚构 capability / inventory entry。该未覆盖需求或 capability gap **可以继续记录在当前 Task Sheet 中，但应位于 Stage 5 plan items 区域之外**，用于保留当前执行上下文和后续处理状态。
...
Task Sheet 中如何表达和维护这类边界外事项，由当前 Agent / 用户结合当前执行范围决定；Stage 5 不为其定义专属 item schema、状态机或工作目录规则。

如果后续通过独立的 capability authoring / update 已形成实际可引用的 `entry`，且当前科研任务仍需要该分析，Stage 5 再在实际承载该分析的 Task Sheet 中重新读取 inventory / `entry`，按正常规则创建相应 plan item。

“记录并学习用户与 Agent 完成 capability gap 时形成的流程，并据此辅助补充 capability”保留为 future update direction；当前 Stage 5 设计与实现不包含该 learning / capability-supplement mechanism。

## 8. Reuse and prepared inputs

### 8.1 `trajectory_index.yaml`

Stage 5 对 `trjconv` / `trjcat` 产生、具有后续复用价值的 trajectory 保留额外的 project-level 登记入口：

```text
<project_root>/05_analysis/indexes/trajectory_index.yaml
```

该索引由对应 trajectory producer capability owner 维护。Stage 5 main Skill 只查询、核验和使用，不负责生成 trajectory、决定集中存放位置、决定哪些 trajectory 文件可登记，或维护该索引。

trajectory producer capability owner 的 Skill / README 应拥有：
...
Stage 5 freeze 只保留索引存在及其消费边界，不复制 producer owner 的具体登记文件白名单。
...
Stage 5 **不建立 project-level `ndx_index.yaml`，也不执行跨 Task Sheet 的 `.ndx` 自动复用扫描/判定**。

同一个 topology / TPR lineage 只能说明体系来源相关，不能证明现有 `.ndx` 已包含当前分析需要的 group。真正判断是否可用仍需检查当前 capability 的 selection/group 语义；考虑到 `.ndx` 文件小且生成成本低，项目级自动 reuse/index 机制没有足够收益。
...
当前 Task Sheet 内如果前置 item 已生成满足后续需求的 `.ndx`，后续多个 items 可以直接共享该 `results`。

同一科研任务的前序 Task Sheet 已明确生成、仍可定位的 `.ndx` 也可以作为当前 item 的候选 prepared input。当前 Task Sheet 必须通过完整路径或明确的前序 Task Sheet / plan item 来源定位该文件；是否包含所需 group、是否与当前分析输入的结构 / 原子顺序兼容，由对应 consuming capability 核验。跨 Task Sheet 显式引用不属于 project-level 自动 reuse，也不要求重新生成或复制 `.ndx`。

用户另外提供的现有 `.ndx` 同样可以作为候选输入；满足当前 consuming capability 的要求则使用，不满足则在当前 plan 中生成新的 index。
...
reuse as input
→ 旧结果可作为当前新 plan item 的输入，避免重复前置分析，但当前 Task Sheet 仍需要新的处理
...
只有对应 capability owner 确认当前执行输出有效后，实际执行的相关 plan item 才进入 `已完成`，关键正式产物才按需写入 `results`。direct reuse item 不因当前 Task Sheet 未执行而进入 `已完成`；它保持 `已终止`，并引用此前已经 validation 的正式结果。
...
项目级登记仍应能够追溯到对应 Task Sheet / Stage 5 plan item，并定位当前 capability、inputs、settings、状态以及被登记的白名单结果文件。Task Sheet 的 `results` 可以比 project-level 白名单更宽，用于当前 Task Sheet 恢复与依赖；`project_result_index.md` 只保留各 capability 明确允许登记的正式结果。

trajectory 的额外集中登记规则由对应 producer capability owner 定义；`trajectory_index.yaml` 与 `project_result_index.md` 是不同用途的登记入口，不由 Stage 5 main Skill 把两者合并成统一文件清单。

是否额外汇总多个分析结果、综合解释或生成报告，由当前用户要求决定，不是 Stage 5 固定完成职责。

## 11. Stage 5 Skill completion boundary

Stage 5 main Skill 只判断**自己负责的当前 Task Sheet Stage 5 plan items** 是否已经处理到可结束当前 Skill 职责的状态，不负责据此宣称整个 `5 Analysis`、当前 Task Sheet 或整个科研任务已完成。
...
当前 Task Sheet 中如果仍有位于 Stage 5 plan items 区域之外的 capability gap、其它边界外事项或未解决要求，当前 Task Sheet 的 `5 Analysis` 是否继续保持未完成，由当前 Task Execution Agent / 用户结合当前执行范围判断。Stage 5 main Skill 不为这些边界外事项定义完成判据，也不因自己的 plan items 已处理完就自动宣称整个 Stage 5、当前 Task Sheet 或科研任务完成。
...
- 由 Stage 5 main Skill 集中定义各 capability 的 `project_result_index.md` 文件白名单；
...
- 由 Stage 5 main Skill 根据自己 plan items 的状态直接判定整个 `5 Analysis`、当前 Task Sheet 或科研任务已完成；
