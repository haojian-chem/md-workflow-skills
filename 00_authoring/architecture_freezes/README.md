...
Stage 2 的阶段级架构 authority 为 `WORKFLOW2_STAGE2_ARCHITECTURE_FREEZE_AND_LINKED_ITP_HANDOFF.md`。当前该 freeze 仍只作为尚未被其它 current freeze / active Skill 取代的 Stage-level architecture authority。当前 2.1–2.6 已生成 active Skill；Stage 2 main Skill 尚未生成 active Skill。

**Topology validation 的 current runtime authority 为 `02_topology_preparation/2.6_topology_validation/SKILL.md` 及其 local references。** `WORKFLOW2_STAGE2_2.6_TOPOLOGY_VALIDATION_FREEZE.md` 已转为 implemented architecture record，不再维护平行 runtime specification。`WORKFLOW2_STAGE2_ARCHITECTURE_FREEZE_AND_LINKED_ITP_HANDOFF.md` 中与 topology validation 的具体依赖、检查内容、结果记录和 GROMACS 预处理有关的旧文本已经被 active Skill package 取代；只有需要 Stage-level architecture 时才读取旧综合冻结中仍未被 current authority 取代的 Stage-level 内容。
...
Stage 3 的 current architecture record 为 `WORKFLOW3_STAGE3_ARCHITECTURE_FREEZE.md`。Stage 3 不设置编号化 sub-stage；周期盒构建、溶剂添加和离子添加由 Stage-level main Skill 在执行期作为内部 operations 规划和维护。原 `3.1 / 3.2 / 3.3` step-level freezes 已移入 `00_authoring/archive/stage3_history/`，不再属于 current authoring path。Current runtime authority 为 `03_md_preparation/SKILL.md`、`03_md_preparation/references/results.md` 和 `03_md_preparation/references/genion.mdp`；该 freeze 已转为 implemented architecture record，不再维护平行 runtime specification。
