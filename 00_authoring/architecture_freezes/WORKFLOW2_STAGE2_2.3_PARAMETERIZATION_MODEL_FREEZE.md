...
本文件记录 `2.3 Topology-linked nonstandard parameterization` 中已经敲定的参数化模型输入、一般截取规则和体系专项规则，作为后续继续设计和正式 Skill generation 的 authoring input。
...
核酸及其它适用体系的具体截取与封端规则继续在上述一般规则下分别讨论和冻结。
