...
只有以下项目通过，才适合把某个 Skill 交给独立网页窗口。

## 启动入口

- [ ] 已读取 `AGENTS.md`；
- [ ] 已读取 `00_authoring/SKILL.md`；
- [ ] 已读取当前负责的目标 Skill / 文件；
- [ ] 没有把整个 `00_authoring/` 当作固定 preload；
- [ ] 目标 content map、freeze、上下游 Skill、Tool guide 仅按当前任务需要继续读取。
...
- [ ] content map 已按需确认；
- [ ] 涉及并行写入时 `write_paths` 已确认且无重叠。

## 读取上下文

- [ ] 已按需读取直接上游 Skill；
- [ ] 已按需读取直接下游 Skill；
- [ ] 已读取当前输入/输出真正依赖的 supporting Skill / Tool guide；
- [ ] 已检查相邻职责是否存在重复或冲突；
...
- [ ] current 文件已不再引用旧路径；
- [ ] 需要保留的旧文件已移入 `00_authoring/archive/`；
