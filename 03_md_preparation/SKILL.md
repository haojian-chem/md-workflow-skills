...
执行 Agent 根据当前处理对象、当前任务上下文以及项目记录中的正式结果检索信息，定位与该对象对应的：
...
当 `gmx solvate -p` 或 `gmx genion -p` 需要修改 `[ molecules ]` 时，先在当前操作目录形成可修改的派生体系主 `.top`。新增溶剂或离子所需的拓扑 / 参数定义如果尚不能由该 `.top` 解析，则根据用户要求或当前体系采用的实际模型补充对应定义或引用。
...
根据用户要求确定溶剂模型及其拓扑 / 参数定义；用户未指定时，根据当前体系和已经采用的参数体系判断。若存在多个会实质改变体系组成或参数化方式的合理选择且无法可靠确定，向用户确认。
...
根据用户要求确定离子种类及其拓扑 / 参数定义；用户未指定时，根据当前体系和已经采用的参数体系判断。若存在多个会实质改变体系组成或参数化方式的合理选择且无法可靠确定，向用户确认。
