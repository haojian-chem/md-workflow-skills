...
当补充的 group 简单且 membership 已经明确时，Agent 可以直接创建或修改 `.ndx` 文件，例如已有明确 atom-number 列表、少量明确原子的组合，或当前 item 已明确给出的简单范围。是否简单到适合 direct edit 由 Agent 根据当前任务判断，不建立固定复杂度判据。
...
本 capability 不建立 project-level `ndx_index.yaml`，也不执行跨 Task `.ndx` 自动登记/扫描。`.ndx` 作为当前分析所需的 prepared input 使用，不因生成而自动进入 `project_result_index.md`。
