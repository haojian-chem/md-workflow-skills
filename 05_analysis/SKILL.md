---
name: md-analysis
description: Stage 5 Analysis 总 Skill。用于把 Task Sheet 中的 `5 Analysis` 目标展开为 capability-based analysis plan，完成 Stage 5 reuse、依赖与任务目录组织、plan-item 状态维护和正式结果登记；具体分析方法、输出 validation 与可登记文件由对应 capability owner 负责。
---
...
Stage 5 不设置编号化 sub-stage，也不建立 `5.1 Analysis planning and orchestration`。本 Skill 直接读取 Task Sheet 中的 `5 Analysis` 条目，并负责：
...
缺失 capability 的方法设计、实现和 validation 不属于本 Skill。其 Task Sheet 表达与后续处理方式由当前 Task Execution Agent / 用户结合任务决定。
...
→ write the complete current Stage 5 plan items into Task Sheet
→ execute the plan
```

正常执行中不为每个 item 重复一次全局 reuse scan。

如果前置失败、实际产物不满足原计划前提、用户修改需求或其它证据使当前 plan 失效，可以调整尚未完成的后续计划。是否修改既有 item、终止后新增 item 或采用其它处理方式，由当前 Agent / 用户根据实际任务决定；本 Skill只要求历史编号、终止原因和有效依赖保持可追溯。
...
`path`
: 当前 Task 实际执行该 item 时的完整工作/文件目录。direct reuse 且当前 Task 不执行该 item 时可省略，不使用旧结果目录冒充当前 item 工作目录。
...
Direct reuse item 不标记 `已完成`，因为当前 Task 没有执行该分析。候选旧结果自己的 inputs/settings 从其原 Task / result record 追溯，与当前 item 比较。
...
Stage 5 不建立 project-level `ndx_index.yaml`，也不做跨 Task `.ndx` 自动复用扫描。
...
同一 Task 中前置 item 生成且满足后续需求的 `.ndx` 可以通过显式依赖共享。
...
本 Skill只核验 orchestration 一致性，不重新做具体 capability 的科学 validation。
...
```text
path    → 当前 Task 实际执行 item 的工作/文件目录与恢复入口
...
- 当前 Stage 5 plan items 中没有仍需本 Skill继续推进的 `未完成` item；
...
Task Sheet 中如果仍有位于 Stage 5 plan items 区域之外的 capability gap、其它边界外事项或未解决任务要求，整个 `5 Analysis` / Task 是否仍为未完成，由当前 Task Execution Agent / 用户判断。本 Skill不因自己的 plan items 已结束就自动宣称整个 Stage / Task 完成。
