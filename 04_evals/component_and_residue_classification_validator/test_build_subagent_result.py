...
def task_document(tmp_path: Path) -> dict:
    return {
        ...
        "permissions": {
            ...
            "forbidden_paths": [
                str(tmp_path / "00_project_state"),
                str(tmp_path / "00_project_records"),
            ],
        },
        "current_valid_files": [],
...
def test_wrapper_builds_clear_validator_result(tmp_path: Path) -> None:
    ...
    output = yaml.safe_load(
        (tmp_path / "subagent_result.yaml").read_text(encoding="utf-8")
    )
    assert output["status"] == "DONE"
    assert output["task_unit_mode"] == "VALIDATOR"
    assert output["operation_result"] is None
    ...
    assert len(output["validation_result"]["created_files"]) == 3


def test_wrapper_converts_pending_requests_to_blocking_confirmation_items(
    tmp_path: Path,
) -> None:
    request = {
        "request_index": 1,
