...
当前 2.5 工作项必须显式记录本次整合实际采用的全部 2.2、2.3、2.4 前置工作项及其正式结果完整路径。同一处理环节存在多个前置工作项时逐项记录，不得以处理环节编号、目录扫描结果或“最新结果”代替具体正式结果引用。能够直接参与整合的 `SOLVENT_COMPONENT` / `ION_COMPONENT` 拓扑定义来源也记录在当前 2.5 工作项中。
...
- 建立或保留一个 2.5 工作项，逐项记录本次整合实际采用的全部 2.2、2.3、2.4 前置工作项及其正式结果完整路径；同一处理环节存在多个前置工作项时分别记录；同时记录能够直接进入 2.5 的 `SOLVENT_COMPONENT` / `ION_COMPONENT` 参数定义来源；
