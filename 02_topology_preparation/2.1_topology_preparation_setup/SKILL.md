...
当前拓扑整合工作项必须显式记录本次整合所依赖的全部标准残基拓扑生成、topology-linked 非标准残基参数化和独立非标准参数化前置工作项。同一类前置任务存在多个工作项时逐项记录，不得仅以任务类型代替具体工作项引用。能够直接参与整合的 `SOLVENT_COMPONENT` / `ION_COMPONENT` 拓扑定义来源也记录在当前拓扑整合工作项中。
...
- 建立或保留一个拓扑整合工作项，逐项记录本次整合所依赖的全部标准残基拓扑生成、topology-linked 非标准残基参数化和独立非标准参数化前置工作项；同一类前置任务存在多个工作项时分别记录；同时记录能够直接参与整合的 `SOLVENT_COMPONENT` / `ION_COMPONENT` 参数定义来源；
