...
- 每个已观察残基都有明确的多构象检查结果；
- 对按 `classification_rules.md` 应执行重原子组成与命名检查的已观察残基，必须已经使用实际参考完成比较；参考文件尚未明确或不可用时不能形成正式 COMPLETE 结果；
- 只有多构象本身无法可靠拆分为候选原子集合时，才允许该残基的重原子比较以 `NOT_PERFORMED` 记录实际原因；整个缺失残基以及按规则不适用的普通溶剂/离子使用 `NOT_APPLICABLE`；
