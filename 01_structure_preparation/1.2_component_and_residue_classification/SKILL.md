...
本 Skill 只定义 1.2 自身的科学检查、复用、validation、结果接口与正式结果规则。
...
分类依据必须能够追溯到实际项目定义、目标力场残基定义、Skill 内置残基 registry 或结构实体语义；不能仅依据 `ATOM / HETATM`、残基名外观或空间位置猜测分类。

### 3. 检查缺失残基

在重原子组成与命名检查之前完成缺失残基检查。

只根据能够追溯的序列或结构注释判断预期存在但当前没有坐标的残基；不得仅根据残基编号间断、chain break 或视觉上的坐标缺口推断缺失残基。
...
记录存在多构象的残基、实际 `altLoc` ID、受影响原子名称及可读取的 occupancy 信息。多构象检查是独立检查项目，不属于重原子组成与命名检查的子项。
...
- 同一实际比较原子集合内的重复 atom name；
- 参考定义中存在、当前残基缺失的重原子；
- 当前残基存在、参考定义中没有的额外重原子；
- atom name 差异以及有明确依据的原子名称对应候选；
...
- 缺失、额外、重复、原子名称对应和元素不一致等发现按正式问题类型记录，没有用模糊的“兼容 / 不兼容”替代实际比较关系；
